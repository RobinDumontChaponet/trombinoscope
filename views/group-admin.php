...
<section id="content">
<?php
echo '<header><form action = "index.php?requ=group&id='.$group->getId().'"><h1>Groupe <input type="text" name="name" value="'.$group->getName().'"></h1><span class="date">(<input type="text" name="date" value="'.$group->getDateString().'">)</span><div><input type="submit" value="Enregistrer"><a href="?requ=suppr&id='.$group->getId().'" onclick="suppr(this); return false" title="supprimer"><span>Supprimer</span></a></div></form></header>';
?>
  <figure class="add">
